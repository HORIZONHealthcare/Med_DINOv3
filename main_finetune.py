...
def get_args_parser():
    parser = argparse.ArgumentParser('MAE fine-tuning / linear probing for image classification', add_help=False)
    # Core training
    ...
    # Model parameters
    parser.add_argument('--model', default='med_dinov3', type=str, metavar='MODEL',
    ...
    parser.add_argument('--finetune', default='', type=str, help='Custom checkpoint path to overlay on hub weights')
...
def main(args, criterion):
    # Load args from a resume (if training) but keep the resume path
    if args.resume and not args.eval:
        resume_path = args.resume
        checkpoint = torch.load(args.resume, map_location='cpu')
        print("Load checkpoint (args) from:", args.resume)
    ...
    # ======= Overlay custom weights on top of hub (if requested & not eval-only) =======
    if args.finetune and not args.eval:
        print(f"Preparing to overlay custom checkpoint on top of hub weights: {args.finetune}")
        misc.overlay_custom_ckpt_on_hub(
            model,
            custom_ckpt_path=args.finetune,
            exclude_patterns=("bias_mask", "attn_mask", "qkv.bias_mask"),
            input_size=args.input_size,
        )

        # Re-init classifier head (linear) for your task
    ...
    if not args.eval:
        ...
        print('len of train_set:', len(data_loader_train) * args.batch_size)
    ...
    # Resume weights for eval-only
    if args.resume and args.eval:
        checkpoint = torch.load(args.resume, map_location='cpu')
        print("Load checkpoint for eval from:", args.resume)
        model.load_state_dict(checkpoint['model'])

    model.to(device)
    model_without_ddp = model

    # ======= Adaptation toggle: ft vs lp =======
    ...
    # Build optimizer parameter groups AFTER freezing
    ...
    if 'convnext' in args.model_arch:
        ...
        for n, p in model_without_ddp.named_parameters():
            ...
            if (p.ndim == 1) or (n in no_weight_decay):
                ...
                decay.append(p)
        param_groups = [
            {"params": decay,    "weight_decay": args.weight_decay, "lr_scale": 1.0},
            {"params": no_decay, "weight_decay": 0.0,               "lr_scale": 1.0},
        ]
        print("[LRD] ConvNeXt: using 2 param groups.")
    else:
        param_groups = misc.param_groups_lrd(
            model_without_ddp,
    ...
    # If a previous full state exists, load it (optimizer, scaler, etc.)
    ...
    if args.eval:
        if 'checkpoint' in locals() and isinstance(checkpoint, dict) and 'epoch' in checkpoint:
            print("Test with the best model at epoch =", checkpoint['epoch'])
    ...
    model.to(device)
    print("Test with the best model, epoch =", checkpoint.get('epoch', -1))
